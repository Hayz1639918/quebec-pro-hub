...
- **Mode TEST activé (2026-07-03) à la demande de l'utilisateur (pas encore de domaine, juste une URL Vercel de test)** : `mailer_autoconfirm = true` → les inscriptions créent le compte + session immédiate SANS email, avec n'importe quelle adresse. Flux inscription→connexion vérifié OK. ⚠️ RÉGLAGE DE TEST À RÉTABLIR AVANT PROD : remettre `mailer_autoconfirm = false` une fois un domaine Resend vérifié, sinon aucune vérification d'email (n'importe qui peut créer un compte avec une fausse adresse). Le code Auth.tsx gère déjà les deux cas (session immédiate vs confirmation requise). Compte de test créé puis supprimé via service_role — base à 33 profils, propre.
