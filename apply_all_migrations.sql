-- Script pour appliquer toutes les migrations en une seule fois
-- ATTENTION: Exécuter ce script seulement si les migrations n'ont pas encore été appliquées
-- Date: 2025-10-22

-- ========================================
-- MIGRATION 001: Profiles table
-- ========================================
-- (Déjà appliquée normalement)

-- ========================================
-- MIGRATION 002: Marketplace fields
-- ========================================
-- (Déjà appliquée normalement)
...
-- ========================================
-- MIGRATION 004: Professional filters
-- ========================================
-- (Déjà appliquée normalement)

-- ========================================
-- MIGRATION 005: Geolocation and activity
-- ========================================
-- (Déjà appliquée normalement)
...
-- ========================================
-- MIGRATION 007: Messaging and notifications
-- ========================================
-- (Déjà appliquée normalement)

-- ========================================
-- MIGRATION 008: Contracts system
-- ========================================
-- (Déjà appliquée normalement)
...
CREATE TABLE IF NOT EXISTS contract_audit_trail (
  ...
  created_at TIMESTAMPTZ DEFAULT NOW(), -- Corrigé: timestamp -> created_at
...
CREATE INDEX IF NOT EXISTS idx_audit_trail_created_at ON contract_audit_trail(created_at); -- Corrigé: timestamp -> created_at
...
RETURNS TABLE (
  ...
  event_type VARCHAR,
  user_name VARCHAR,
  created_at TIMESTAMPTZ, -- Corrigé: timestamp -> created_at
  ip_address VARCHAR,
  details JSONB
) AS $$
BEGIN
  ...
  SELECT 
    ...
    cat.created_at, -- Corrigé: timestamp -> created_at
    cat.ip_address::VARCHAR,
    cat.details
  FROM contract_audit_trail cat
  WHERE cat.contract_id = contract_uuid
  ORDER BY cat.created_at DESC; -- Corrigé: timestamp -> created_at
END;
...
CREATE OR REPLACE FUNCTION add_audit_trail_entry(
  ...
  created_at_param TIMESTAMPTZ, -- Corrigé: timestamp -> created_at
...
BEGIN
  INSERT INTO contract_audit_trail (
    contract_id, event_type, user_id, user_name, created_at, -- Corrigé: timestamp -> created_at
    ip_address, user_agent, details
  ) VALUES (
    contract_uuid, event_type_param, user_id_param, user_name_param, created_at_param, -- Corrigé: timestamp -> created_at
...
-- ========================================
-- MIGRATION 011: Contract milestones
-- ========================================
-- (Déjà appliquée normalement)
...
RETURNS VOID AS $$
BEGIN
  -- Notify about RBQ expiry (30 days before)
  ...
  -- Notify about insurance expiry (30 days before)
  INSERT INTO notifications (user_id, type, title, message, metadata)
