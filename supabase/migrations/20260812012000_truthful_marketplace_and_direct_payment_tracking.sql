...
SELECT
  ...
  p.total_reviews,
  p.total_projects,
  NULLIF(p.total_projects_external, p.total_projects_external) AS total_projects_external,
  NULLIF(p.business_volume_cad, p.business_volume_cad) AS business_volume_cad,
  ...
  NULLIF(p.last_active_at, p.last_active_at) AS last_active_at,
  NULLIF(p.total_proposals_sent, p.total_proposals_sent) AS total_proposals_sent,
  NULLIF(p.proposals_last_30_days, p.proposals_last_30_days) AS proposals_last_30_days,
  NULLIF(p.profile_views_count, p.profile_views_count) AS profile_views_count,
  NULLIF(p.profile_views, p.profile_views) AS profile_views,
  NULLIF(p.activity_score, p.activity_score) AS activity_score,
  p.hourly_rate_min,
  p.hourly_rate_max,
  p.daily_rate_min,
  p.daily_rate_max,
  ...
  NULLIF(p.favorites_count, p.favorites_count) AS favorites_count,
  p.service_radius_km,
  p.rbq_subcat,
  p.trade_specialty,
  p.certifications,
  ...
  p.service_zones,
  p.professional_type,
  NULLIF(p.subscription_tier, p.subscription_tier) AS subscription_tier,
