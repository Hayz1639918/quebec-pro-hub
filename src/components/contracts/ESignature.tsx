...
interface ESignatureProps {
  onSignatureComplete: (signatureData: SignatureSubmission) => Promise<SignatureData>;
  signerName: string;
...
export const ESignature = ({
  ...
  const saveSignature = async () => {
    ...
    try {
      ...
      // Evidence such as timestamps, hashes, IP address and the verification
      // code is generated by the trusted signing backend, never the browser.
      const submission = await signatureService.createSignatureSubmission(
        signatureImage,
        lastPoint || { x: 0, y: 0 },
        signatureMode,
      );
      const savedSignature = await onSignatureComplete(submission);

      toast({
        title: t('contracts.signature.signed'),
        description: t('contracts.signature.signed_description', { 
          signer: signerName,
          code: savedSignature.verification_code,
        }),
      });
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      const clientSignature = contractData.client_signature_data as unknown as SignatureData | null;
      const professionalSignature = contractData.professional_signature_data as unknown as SignatureData | null;

      await generateSignedContractPDF(contract, clientSignature, professionalSignature);

      await signatureService.recordAuditEvent(contract.id, 'downloaded', {
        download_type: 'pdf',
      });
